...
import { View, Text, TouchableOpacity, ScrollView, TextInput, Platform, StatusBar, Modal, TouchableWithoutFeedback, KeyboardAvoidingView } from 'react-native';
...
export default function JoinClassScreen({ navigation }: any) {
    ...
    // --- Assessment Modal States (For Memorization Classes) ---
    ...
    // 1. Initial click on "Join"
    const initiateJoinRequest = (item: AvailableClass) => {
        if (item.category === 'memorization') {
            // Open assessment modal for memorization classes
            setSelectedClassId(item.id);
            setPreviousMemorization('');
            setDailyCapacity('نصف صفحة');
            setIsAssessmentModalVisible(true);
        } else {
            // Direct join for tajweed classes
            setRequestedClasses((prev) => ({
                ...prev,
                [item.id]: true,
            }));
            setSuccessMessage('تم إرسال طلب الانضمام لحلقة التجويد بنجاح. سيتم إشعارك عند القبول.');
            setIsSuccessModalVisible(true);
        }
    };

    // 2. Submit form from assessment modal
    const submitAssessmentJoinRequest = () => {
        if (selectedClassId) {
            setRequestedClasses((prev) => ({
                ...prev,
            ...
            setSuccessMessage('تم إرسال طلب الانضمام مع معلومات حفظك للمُعلم بنجاح. سيتم إشعارك عند القبول بناءً على التقييم.');

            // استخدام مهلة بسيطة لضمان إغلاق النافذة الأولى بسلام قبل فتح نافذة النجاح
            setTimeout(() => {
                setIsSuccessModalVisible(true);
            }, 300);
        }
    ...
    return (
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} className="flex-1 bg-background">
            <View style={{ paddingTop: Platform.OS === 'android' ? StatusBar.currentHeight : 40 }} className="flex-1">
                    ...
                    <View className="px-5 space-y-4">
                        {filteredClasses.length > 0 ? (
                            filteredClasses.map((item) => {
                                ...
                                const isRequested = requestedClasses[item.id];
                                    ...
                                    <View key={item.id} className="bg-white rounded-2xl p-5 border border-border shadow-sm mb-4">
                                        ...
                                        <View className="flex-row-reverse items-center justify-between mt-2">
                                            ...
                                            <TouchableOpacity
                                                ...
                                                onPress={() => initiateJoinRequest(item)}
                                                activeOpacity={0.8}
                ...
                {/* --- Assessment Modal (For Memorization Classes) --- */}
                <Modal
                    animationType="fade"
                    transparent={true}
                ...
                >
                    <View className="flex-1 justify-center items-center px-5 bg-black/50">
                        <View className="bg-white w-full rounded-3xl p-6 shadow-2xl">
                            <View className="flex-row-reverse items-center justify-between mb-4 border-b border-gray-100 pb-4">
                                <Text className="text-lg text-foreground" style={{ fontFamily: 'Tajawal-Bold' }}>معلومات الحفظ السابقة</Text>
                                <TouchableOpacity onPress={() => setIsAssessmentModalVisible(false)} className="bg-gray-50 p-2 rounded-full">
                                    <Feather name="x" size={20} color="#64748b" />
                                </TouchableOpacity>
                            </View>

                            <Text className="text-sm text-slate-500 mb-6 text-center leading-relaxed" style={{ fontFamily: 'Tajawal-Medium' }}>
                                لكي يقوم المعلم بإعداد خطة تتناسب مع مستواك، يرجى تزويدنا ببعض المعلومات السريعة.
                            </Text>

                            <Text className="text-sm text-slate-700 mb-2 text-right" style={{ fontFamily: 'Tajawal-Bold' }}>ما هو مقدار حفظك السابق؟</Text>
                            <TextInput
                                className="bg-gray-50 border border-gray-200 rounded-xl h-12 px-4 text-right text-sm text-foreground mb-6 focus:border-primary"
                                placeholder="مثال: جزء عم وتبارك فقط"
                                placeholderTextColor="#9ca3af"
                                value={previousMemorization}
                                onChangeText={setPreviousMemorization}
                                style={{ fontFamily: 'Tajawal-Medium' }}
                            />

                            <Text className="text-sm text-slate-700 mb-2 text-right" style={{ fontFamily: 'Tajawal-Bold' }}>كم تستطيع أن تحفظ يومياً؟</Text>
                            <View className="flex-row-reverse flex-wrap gap-2 mb-8">
                                {['ربع صفحة', 'نصف صفحة', 'صفحة كاملة', 'صفحتين'].map(cap => (
                                    <TouchableOpacity
                                        key={cap}
                                        onPress={() => setDailyCapacity(cap)}
                                        activeOpacity={0.8}
                                        className={`px-4 py-2.5 rounded-xl border transition-colors ${dailyCapacity === cap ? 'bg-primary border-primary shadow-sm' : 'bg-white border-gray-200'}`}
                                    >
                            ...
                            <TouchableOpacity
                                onPress={submitAssessmentJoinRequest}
                                activeOpacity={0.8}
                                className="w-full bg-primary h-14 rounded-xl items-center justify-center shadow-sm"
                            >
                                <Text className="text-white text-base" style={{ fontFamily: 'Tajawal-Bold', includeFontPadding: false, marginTop: 4 }}>تأكيد طلب الانضمام</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                </Modal>
                ...
                <Modal animationType="fade" transparent={true} visible={isSuccessModalVisible} onRequestClose={() => setIsSuccessModalVisible(false)}>
                    <View className="flex-1 justify-center items-center px-6 bg-black/50">
                        <View className="bg-white w-full rounded-3xl p-6 items-center shadow-2xl">
                            ...
                            <TouchableOpacity onPress={() => setIsSuccessModalVisible(false)} activeOpacity={0.8} className="w-full h-12 bg-primary rounded-xl items-center justify-center shadow-sm">
